...
[cite\_start]核心任务是重构Optiland的光线追踪引擎，引入一个“传播模型” (`PropagationModel`) 抽象层 [cite: 1, 2][cite\_start]。在此基础上，将现有的直线传播逻辑封装成`HomogeneousPropagation`模型 [cite: 5][cite\_start]，并实现一个新的`GrinPropagation`模型来支持梯度折射率(GRIN)介质 [cite: 11]。
...
## 2\. 受影响的核心模块与文件 (Impact Analysis)
  ...
  - [cite\_start]`optiland/rays.py` (**理解/调用**): `RealRays`对象是整个追踪过程的核心数据结构，所有传播模型都将对其进行操作 [cite: 2, 5, 11]。
  - [cite\_start]`optiland/surfaces/base.py` (**理解/调用**): `BaseSurface`是所有表面的基类，其接口（如`material_post`属性）是连接交互和传播的关键 [cite: 3, 18]。
  - [cite\_start]`optiland/materials/base.py` (**理解/调用**): `BaseMaterial`是所有材料的基类，`GradientMaterial`将继承自它 [cite: 10]。
...
### 模块/文件 A (修改): `optiland/optic.py`
  ...
  - **与任务的关联**: 此文件包含核心的`Optic.trace`方法，即光线追踪主循环。当前，它将光线-表面交互和介质中传播的逻辑耦合在一起。**我们的目标是解耦这个循环**：保留表面交互部分（`surface.trace(rays)`），并将传播部分（`intersect`、更新`x,y,z`、更新`opd`）委托给新的`PropagationModel`。
        ...
        def trace(self, rays: rays.RealRays, report_progress: bool = False) -> rays.RealRays:
            ...
            for i, surface in enumerate(iterable_surfaces):
                ...
                if i > 0:
                    ...
                    # [cite_start]它将被 HomogeneousPropagation.propagate 封装 [cite: 5, 6, 7, 8]
                    [cite_start]distance = surface.geometry.intersect(rays) # [cite: 6]
                    [cite_start]rays.x += distance * rays.L # [cite: 7]
                    [cite_start]rays.y += distance * rays.M # [cite: 7]
                    [cite_start]rays.z += distance * rays.N # [cite: 7]
                    [cite_start]n = self.surfaces[i-1].material_post.n(rays.w) # [cite: 8]
                    [cite_start]rays.opd += n * distance # [cite: 8]

                # apply surface (这部分保留不变)
  ...
  - **交互与依赖**: `Optic.trace`方法目前直接依赖`geometry.intersect()`和`material_post.n()`。重构后，它将不再直接调用`intersect`，而是依赖`PropagationModel.propagate`。它还需要导入`GradientMaterial`来进行`isinstance`类型检查。

### 模块/文件 B (理解/调用): `optiland/rays.py`

  - [cite\_start]**与任务的关联**: `RealRays`是贯穿整个系统的核心数据结构。所有`PropagationModel`的`propagate`方法的输入和输出都是`RealRays`对象 [cite: 2, 5, 11]。
...
### 模块/文件 C (理解/调用): `optiland/surfaces/base.py`
  ...
  - [cite\_start]**与任务的关联**: `BaseSurface`定义了光学表面的通用接口。`PropagationModel`将使用`surface_in`和`surface_out`作为其边界 [cite: 2, 3]，并通过`surface_in.material_post`来确定介质类型。
...
1.  **创建传播模型抽象层**:
      ...
      * 在`optiland/propagation/base.py`中，创建`PropagationModel(ABC)`抽象基类，并定义`@abstractmethod propagate(self, rays_in: RealRays, surface_in: BaseSurface, surface_out: BaseSurface) -> RealRays:`。

2.  **封装现有逻辑 (均质介质)**:

      * 在`optiland/propagation/homogeneous.py`中，创建`HomogeneousPropagation(PropagationModel)`类。
      * [cite\_start]将其`propagate`方法的实现逻辑，直接从`optiland/optic.py`的`if i > 0:`代码块中**剪切**过来（即计算`distance` [cite: 6] [cite\_start]并更新`x, y, z` [cite: 7] [cite\_start]和 `opd` [cite: 8] 的部分）。

3.  **实现GRIN传播模型**:

      * 在`optiland/propagation/gradient.py`中，创建`GrinPropagation(PropagationModel)`类。
      * [cite\_start]此类应在`__init__`中接受`step_size`和`max_steps` [cite: 12]。
      * [cite\_start]其`propagate`方法应通过`surface_in.material_post`获取`GradientMaterial` [cite: 13, 14]，然后调用（可设为私有的）RK4数值积分求解器。

4.  **重构核心追踪引擎 (`Optic.trace`)**:

      * 在`optiland/optic.py`顶部，导入`GradientMaterial`, `HomogeneousPropagation`, `GrinPropagation`。
      * **修改`trace`方法的核心循环** (替换`if i > 0:`代码块)：
        ```python
        # (在 'trace' 方法的 'for' 循环内部)
        if i > 0:
            surface_in = self.surfaces[i-1]
            # 1. 获取两个表面之间的介质
            medium = surface_in.material_post
            
            # 2. 根据介质类型选择传播模型
            if isinstance(medium, GradientMaterial):
                # (建议：在Optic.__init__中实例化模型以重用)
                propagation_model = GrinPropagation() 
            else:
                propagation_model = HomogeneousPropagation()
            
            # [cite_start]3. 调用模型进行传播 [cite: 18]
            rays = propagation_model.propagate(rays, surface_in, surface)
...
5.  **创建GRIN相关模块**:
      ...
      * [cite\_start]根据设计文档，创建`optiland/materials/gradient_material.py` [cite: 10] [cite\_start]和`optiland/surfaces/gradient_surface.py` [cite: 9] 文件，并添加相应的类定义。

## 5\. 测试与集成上下文 (Testing & Integration Context)

  - **测试模式**: 项目使用 `pytest`。测试文件位于 `tests/` 目录下（例如`tests/test_optic.py`）。测试风格偏向于构建小型的、完整的`Optic`系统并验证端到端的追踪结果。

  - **相关测试示例**: (摘自 `tests/test_optic.py`，用于展示集成测试风格)
