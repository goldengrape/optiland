...
def propagate_through_gradient(
    ...
    # Assuming rays_in.w holds the wavelength information
    ...
    # --- MODIFIED TYPE HINTS ---
    # Replaced be.ndarray with Any and Tuple[...] with tuple to avoid runtime TypeError.
    # be.ndarray is a tuple of types for isinstance checks, not a valid type for annotation.
    def derivatives(current_r: Any, current_k: Any, wavelength: Any) -> tuple:
        n, grad_n = grin_material.get_index_and_gradient(current_r[:, 0], current_r[:, 1], current_r[:, 2], wavelength)
    ...
    for i in range(max_steps):
        ...
        n_current = grin_material.get_index(r_active[:, 0], r_active[:, 1], r_active[:, 2], w_active)
        ...
        # Refined State Management Logic
        # 1. Immediately update the main state array for all active rays.
        r[active_rays] = r_next_active
        k[active_rays] = k_next_active
        
        # 2. Accumulate Optical Path Difference (OPD) for active rays
        n_next = grin_material.get_index(r_next_active[:, 0], r_next_active[:, 1], r_next_active[:, 2], w_active)
        opd[active_rays] += 0.5 * (n_current + n_next) * step_size

        # 3. Check for intersection within the segment [r_active, r_next_active]
        ...
        if be.any(intersected_mask_local):
            ...
            # Calculate exact intersection point for these specific rays
            intersection_point = r_active[intersected_mask_local] + distance_to_intersect[intersected_mask_local, be.newaxis] * segment_dir[intersected_mask_local]
            
            # Store their final state
            ...
            # Deactivate these rays for subsequent iterations
    ...
    w_final = rays_in.w
    n_final = grin_material.get_index(final_r[:, 0], final_r[:, 1], final_r[:, 2], w_final)
