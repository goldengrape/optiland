...
class TorchBackend(AbstractBackend):
    ...
    def _grad_for_dtype(self, dtype: torch.dtype) -> bool:
        """Return whether tensors of this dtype may require gradients."""
        return self._grad() and (dtype.is_floating_point or dtype.is_complex)
    ...
    def array(self, x: ArrayLike, dtype: Any = None) -> Tensor:
        ...
        if isinstance(x, torch.Tensor):
            return x.to(dtype=resolved_dtype) if dtype is not None else x
        ...
        if isinstance(x, (list, tuple)) and len(x) > 0:
            ...
            if any(isinstance(v, torch.Tensor) for v in x):
                ...
                tensors = [
                    ...
                    else torch.tensor(v, device=self._device(), dtype=resolved_dtype)
                    for v in x
                ]
                if dtype is not None:
                    tensors = [t.to(dtype=resolved_dtype) for t in tensors]
        ...
        return torch.tensor(
            ...
            requires_grad=self._grad_for_dtype(resolved_dtype),
        )
    ...
    def zeros_like(self, x: Any) -> Tensor:
        ...
        return torch.zeros_like(
            ...
            device=self._device(),
            dtype=x_t.dtype,
            requires_grad=self._grad_for_dtype(x_t.dtype),
        )
